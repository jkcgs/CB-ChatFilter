...
public class ChatFilterCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(args.length < 1) {
			...
			if(sub.equals("help")) {
				...
				help.add(String.format("/%s help - %s", cmd.getName(), p.lang.str("helpDescription")));
				help.add(String.format("/%s reload - %s", cmd.getName(), p.lang.str("reloadDescription")));
				help.add(String.format("/%s save - %s", cmd.getName(), p.lang.str("saveDescription")));
				help.add(String.format("/%s toggle - %s", cmd.getName(), p.lang.str("toggleDescription")));
				help.add(String.format("/%s add - %s", cmd.getName(), p.lang.str("addDescription")));
				help.add(String.format("/%s remove - %s", cmd.getName(), p.lang.str("removeDescription")));
			...
			if(sub.equals("toggle")) {
				...
				if(toggle.equals("global")) {
					...
					// TODO: Lang
					senderLog(sender, "That toggle does not exists. Type '/" + cmd.getName() + " toggle help' for get a toggle list");
				}
	...
	/**
	 * Logs a message to the console. If the sender is a player, send him 
	 * the message too.
	 * @param sender The sender, could be the console.
	 * @param msg The message to log and informate.
	 */
